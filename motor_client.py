...
#
# motor_client.py
# Hacked Makani command line motor client.
# Example:
#   motor_client.py --init --spinup
#
...
import actuator_types
import aio
import cmd_client
import pack_avionics_messages
import safety_codes
import client as param_client
...
GEN_TABLE_PATH = './gen_lookup_table.py'
# odd to find this hardcoded in here like this
...
class MotorCommandClient(cmd_client.WingCommandClient):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self._spin_dir = {}
    # start threads to broadcast messages
    self._motor_runner = Runner(self._motors_selected, self._spin_dir)
    self._dyno_runner = Runner(self._dynos_selected, self._spin_dir, dyno_mode=True)
...
if __name__ == '__main__':
  ...
  parser = OptionParser()
  parser.add_option("-s", "--spinup", action="store_true", dest="spinup")
  ...
  if ( options.init ):
  	# Explicitly disable the stacking controller
  	client.do_set_param("PTI stacking_enable 0")
  	time.sleep(0.1)

  	# do not stop on warnings
  	client.do_set_param("PTI shutdown_on_warning 0")
  	time.sleep(0.1)

  	# set hardcoded specific motor
  	# (perhaps should nmap to determine ipaddr, then lookup motor name?)
  	client.do_set_targets("PTI");
  	time.sleep(0.1)

	# to avoid kMotorErrorBadMode
	client.do_disarm('')
  	time.sleep(0.1)

  	# clear any errors
  	client.do_clear_errors('')
  	time.sleep(0.1)

  	# set speed controller PI params
  	client.do_set_param("PTI omega_to_torque_kp 10")
  	time.sleep(0.1)
  	client.do_set_param("PTI omega_to_torque_ki 150")
  	time.sleep(0.1)

  	# limits to allow full power operation
  	client.do_set_param("PTI phase_current_limit 270")
  	time.sleep(0.1)
  	client.do_set_param("PTI bus_current_limit 200")
  	time.sleep(0.1)
  	client.do_set_param("PTI v_bus_upper_limit 950")
  	time.sleep(0.1)
  	client.do_set_param("PTI v_bus_lower_limit 0")
  	time.sleep(0.1)

  	# Quadrature current command limits
  	client.do_set_param("PTI iq_upper_limit 225")
  	time.sleep(0.1)
  	client.do_set_param("PTI iq_lower_limit -225")
  	time.sleep(0.1)

  	# Bus current command limits
  	client.do_set_param("PTI ibus_upper_limit 195")
  	time.sleep(0.1)
  	client.do_set_param("PTI ibus_lower_limit -195")
  	time.sleep(0.1)
  ...
  if ( options.spinup ):
    # speed ramp to 15 rad/sec in 10 sec
	client.do_ramp_omega("15 10")
	time.sleep(0.1)
  ...
  # all done with hardcoded commands, now process commands from stdin
  try:
    client.cmdloop()
